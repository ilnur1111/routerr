#!/bin/sh
# opera_region_tester.sh — v1.6 (08-Jul-2025)
set -e
...
check_opera() {
  OPERA_BIN=$(command -v opera-proxy) || { echo "opera-proxy not found"; exit 1; }
}
...
run_proxy_tmp() {          # $1 region
  ...
  for i in $(seq 1 12); do
    netstat -tn | grep -q "127\.0\.0\.1:$PORT" && break
    sleep 1
  done
}
