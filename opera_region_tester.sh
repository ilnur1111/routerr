#!/bin/sh
# opera_region_tester.sh — v1.2 (07-Jul-2025)
# Тестирует регионы EU / AM / AS для opera-proxy и сохраняет лучший.
# Совместим с BusyBox/OpenWrt без расширенных опций nc.
...
run_proxy_tmp() {                       # $1 = region
    ...
    # Ожидаем порт через netstat (универсально для BusyBox)
...
patch_init() {                          # $1 = region
    ...
    # удаляем возможные старые -country XX
    sed -i -E 's@-country[[:space:]]+[A-Z]{2}@@' "$INIT"
    # и добавляем актуальный параметр
...
main() {
    ...
    if [ -n "$REG_OVERRIDE" ]; then
        CHOSEN="$REG_OVERRIDE"
        echo "Non-interactive mode: region $CHOSEN chosen via \$REG_OVERRIDE."
    else
