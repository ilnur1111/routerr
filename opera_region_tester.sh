#!/bin/sh
# opera_region_tester.sh — v1.1.1 (07-Jul-2025) совместим с BusyBox nc
set -e
PORT=18080
...
run_proxy_tmp() {                         # $1 region (EU/AM/AS)
  kill_proxy
  "$OPERA_BIN" -country "$1" -bind-address 127.0.0.1:$PORT >/dev/null 2>&1 &
  # BusyBox-совместимое ожидание: nc без -z
  for i in $(seq 1 12); do
      nc -w 1 127.0.0.1 $PORT < /dev/null >/dev/null 2>&1 && break
      sleep 1
  done
}
...
main() {
  need_root; check_opera; get_regions
  for r in $REGIONS; do measure_region "$r"; done
  kill_proxy; print_results
